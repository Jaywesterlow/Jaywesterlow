...
def make(ctx, P):
    ...
    def practical(m):
        ...
        route = '''<svg viewBox="0 0 700 120" style="width:100%;height:auto" role="img" aria-label="Route Utrecht naar Hinterglemm">
          ...
          ''' + ''.join(f'<circle cx="{x}" cy="70" r="{7 if i in (0,5) else 5}" fill="{"var(--ink)" if i in (0,5) else "var(--snow)"}" stroke="var(--ink)" stroke-width="2"/><text x="{x}" y="{50 if i%2==0 else 100}" text-anchor="{anch(i)}" font-family="Instrument Sans" font-size="13" font-weight="{700 if i in (0,5) else 500}" fill="var(--ink)">{n}</text><text x="{x}" y="{36 if i%2==0 else 114}" text-anchor="{anch(i)}" font-family="Instrument Sans" font-weight="500" font-size="11" fill="var(--stone-2)">{d}</text>' for i,(x,n,d) in enumerate(((30,'Utrecht','0 km'),(160,'Keulen','230 km'),(290,'Frankfurt','420 km'),(440,'München','830 km'),(560,'Kufstein','920 km'),(670,'Hinterglemm','1.000 km')))) + '</svg>'
        cols = '1fr' if m else 'repeat(3, minmax(0, 1fr))'
        return f'''<section style="padding:{pad(m)};display:flex;flex-direction:column;gap:{"28px" if m else "48px"}">
